(function(){
  ...
  function startEdit(id){const r=all.find(x=>String(x.id)===String(id));if(!r)return;editId=r.id;$("cementDate").value=r.tarih||iso(new Date());$("cementCompany").value=r.firma||"";$("cementDelivery").value="Şantiye";$("cementVehicleCount").value=r.arac_sayisi??"";$("cementTonnage").value=r.toplam_tonaj==null?"-":r.toplam_tonaj;$("cementSaveBtn").textContent="Değişiklikleri Kaydet";$("cementCancelBtn").classList.remove("hidden");$("cementPage").scrollIntoView({behavior:"smooth",block:"start"})}
  function cancelEdit(){editId=null}
  async function saveEdit(e){if(!editId)return;e.preventDefault();e.stopImmediatePropagation();const raw=String($("cementTonnage").value||"").trim(),ton=raw==="-"||raw===""?null:Number(raw.replace(",",".")),vehicle=Number($("cementVehicleCount").value),payload={tarih:$("cementDate").value,firma:title($("cementCompany").value),teslim_yeri:"Şantiye",arac_sayisi:vehicle,toplam_tonaj:ton};if(!payload.tarih||!payload.firma||!Number.isInteger(vehicle)||vehicle<1||(ton!==null&&(!Number.isFinite(ton)||ton<=0))){const s=$("cementStatus");if(s){s.textContent="Tarih, firma ve araç sayısını kontrol et. Tonaj bilinmiyorsa - bırakabilirsin.";s.style.color="var(--danger)"}return}const client=db();if(!client)return;const {error}=await client.from(TABLE).update(payload).eq("id",editId);if(error){alert("Kayıt güncellenemedi: "+error.message);return}editId=null;$("cementSaveBtn").textContent="Çimento Sevkiyatını Kaydet";$("cementCancelBtn").classList.add("hidden");$("cementCompany").value="";$("cementDelivery").value="";$("cementVehicleCount").value="";$("cementTonnage").value="";await loadAll()}
...
(function loadRecordsCementAddon(){if(document.querySelector('script[src^="./records-cement-addon.js"]'))return;const s=document.createElement("script");s.src="./records-cement-addon.js?v=20260820-contact1";s.defer=true;document.head.appendChild(s)})();
