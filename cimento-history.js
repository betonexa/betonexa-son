(function(){
  ...
  function ensurePalletField(){
    const vehicle=$("cementVehicleCount");if(!vehicle||$("cementPalletCount"))return;
    const vehicleField=vehicle.closest(".cement-field");if(!vehicleField)return;
    const field=document.createElement("div");field.className="cement-field";field.id="cementPalletField";field.innerHTML='<label>Palet sayısı</label><input id="cementPalletCount" type="number" min="0" step="1" inputmode="numeric" placeholder="0" required>';
  ...
  function render(){const rows=$("cementRows");if(!rows)return;ensurePalletHeader();const items=filtered();rows.innerHTML=items.length?items.map((r,i)=>`<tr><td>${i+1}</td><td>${esc(trDate(r.tarih))}</td><td>${esc(title(r.firma))}</td><td>Şantiye</td><td>${Number(r.arac_sayisi||0)}</td><td>${Number(r.palet_sayisi||0)}</td><td>${r.toplam_tonaj==null?"-":fmt(r.toplam_tonaj)+" ton"}</td><td><div class="cement-row-actions"><button type="button" class="icon-action edit" title="Düzenle" data-cement-edit="${esc(r.id)}">✏️</button><button type="button" class="icon-action delete" title="Sil" data-cement-delete="${esc(r.id)}">🗑️</button></div></td></tr>`).join(""):'<tr><td colspan="8" class="cement-empty">Seçilen dönemde çimento sevkiyatı bulunmuyor.</td></tr>';
  ...
  function fillEditForm(r){
    if(!r)return;editId=r.id;ensurePalletField();$("cementDate").value=r.tarih||iso(new Date());$("cementCompany").value=r.firma||"";$("cementDelivery").value="Şantiye";$("cementVehicleCount").value=r.arac_sayisi??"";$("cementPalletCount").value=r.palet_sayisi??0;$("cementTonnage").value=r.toplam_tonaj==null?"0":r.toplam_tonaj;$("cementSaveBtn").textContent="Değişiklikleri Kaydet";$("cementCancelBtn").classList.remove("hidden");$("cementPage").scrollIntoView({behavior:"smooth",block:"start"});
  }
  ...
  function clearCementInputs(){
    editId=null;if($("cementDate"))$("cementDate").value=iso(new Date());["cementCompany","cementDelivery","cementVehicleCount","cementPalletCount","cementTonnage"].forEach(id=>{if($(id))$(id).value=""});if($("cementSaveBtn"))$("cementSaveBtn").textContent="Sevkiyatı Kaydet";$("cementCancelBtn")?.classList.add("hidden");
  }
  async function saveEdit(e){
    if(!e.target.closest?.("#cementSaveBtn"))return;
    e.preventDefault();e.stopImmediatePropagation();ensurePalletField();
    const raw=String($("cementTonnage")?.value||"").trim(),number=(raw==="-"||raw==="0")?0:Number(raw.replace(",",".")),ton=number===0?null:number,vehicle=Number($("cementVehicleCount")?.value),pallet=Number($("cementPalletCount")?.value),payload={tarih:$("cementDate")?.value,firma:title($("cementCompany")?.value),teslim_yeri:"Şantiye",arac_sayisi:vehicle,palet_sayisi:pallet,toplam_tonaj:ton,tamamlandi:false};
    if(!payload.tarih||!payload.firma||!Number.isInteger(vehicle)||vehicle<1||!Number.isInteger(pallet)||pallet<0||(ton!==null&&(!Number.isFinite(ton)||ton<=0))){const s=$("cementStatus");if(s){s.textContent="Tarih, firma, araç ve palet sayısını kontrol et. Tonaj bilinmiyorsa 0 veya - girebilirsin.";s.style.color="var(--danger)"}return}
  ...
  function wrapBaseEdit(){
    if(typeof window.editCementShipment!=="function"||window.editCementShipment.__paletWrapped)return;
    const original=window.editCementShipment;const wrapped=function(id){original(id);setTimeout(()=>startEdit(id),0)};wrapped.__paletWrapped=true;window.editCementShipment=wrapped;
  }
