(function(){
  "use strict";
  const note=value=>window.BetonexaNoteText(value);
  ...
  function ensureCementTypeHeader(){
    const row=document.querySelector("#cementPage .cement-table-wrap thead tr");if(!row||row.querySelector("[data-cement-type-head]"))return;
    const company=[...row.children].find(th=>th.textContent.trim()==="Firma");if(!company)return;
    const th=document.createElement("th");th.dataset.cementTypeHead="1";th.textContent="Çimento özelliği";company.insertAdjacentElement("afterend",th);
  }
  ...
  function inject(){const page=$("cementPage");if(!page||$("cementFilters"))return false;
    ensurePalletField();ensurePalletHeader();ensureCementTypeField();ensureCementTypeHeader();
  ...
  async function loadAll(){const client=db();if(!client)return;const status=$("cementStatus");if(status)status.textContent="Yükleniyor…";const {data,error}=await client.from(TABLE).select("*").order("tarih",{ascending:false}).order("created_at",{ascending:false});if(error){if(status){status.textContent="Çimento kayıtları alınamadı: "+error.message;status.style.color="var(--danger)"}return}all=data||[];if(status)status.textContent="";ensurePalletField();ensurePalletHeader();ensureCementTypeField();ensureCementTypeHeader();render()}
  function render(){const rows=$("cementRows");if(!rows)return;ensurePalletHeader();ensureCementTypeHeader();const header=rows.closest("table")?.querySelector("thead tr");if(header&&!header.querySelector("[data-notes-header]")){const th=document.createElement("th");th.dataset.notesHeader="1";th.textContent="Not";header.querySelector("th:last-child")?.before(th)}const items=filtered();rows.innerHTML=items.length?items.map((r,i)=>`<tr><td>${i+1}</td><td>${esc(trDate(r.tarih))}</td><td>${esc(title(r.firma))}</td><td>${esc(r.cimento_ozelligi||"-")}</td><td>${esc(r.teslim_yeri||"-")}</td><td>${countText(r.arac_sayisi)}</td><td>${countText(r.palet_sayisi)}</td><td>${r.toplam_tonaj==null?"-":fmt(r.toplam_tonaj)+" ton"}</td><td>${esc(note(r.notlar)||"-")}</td><td><div class="cement-row-actions"><button type="button" class="icon-action edit" title="Düzenle" data-cement-edit="${esc(r.id)}">✏️</button><button type="button" class="icon-action delete" title="Sil" data-cement-delete="${esc(r.id)}">🗑️</button></div></td></tr>`).join(""):'<tr><td colspan="10" class="cement-empty">Seçilen dönemde çimento sevkiyatı bulunmuyor.</td></tr>';
  ...
  function fillEditForm(r){
    if(!r)return;editId=r.id;ensurePalletField();$("cementDate").value=r.tarih||iso(new Date());$("cementCompany").value=r.firma||"";$("cementType").value=r.cimento_ozelligi||"";$("cementDelivery").value=r.teslim_yeri||"";$("cementVehicleCount").value=countText(r.arac_sayisi);$("cementPalletCount").value=countText(r.palet_sayisi);$("cementTonnage").value=r.toplam_tonaj==null?"0":r.toplam_tonaj;$("cementNotes").value=note(r.notlar);$("cementSaveBtn").textContent="Değişiklikleri Kaydet";$("cementCancelBtn").classList.remove("hidden");$("cementPage").scrollIntoView({behavior:"smooth",block:"start"});
  }
  ...
  function clearCementInputs(){editId=null;if($("cementDate"))$("cementDate").value=iso(new Date());["cementCompany","cementType","cementDelivery","cementVehicleCount","cementPalletCount","cementTonnage","cementNotes"].forEach(id=>{if($(id))$(id).value=""});if($("cementSaveBtn"))$("cementSaveBtn").textContent="Sevkiyatı Kaydet";$("cementCancelBtn")?.classList.add("hidden")}
  async function saveEdit(e){
    if(!e.target.closest?.("#cementSaveBtn"))return;
    e.preventDefault();e.stopImmediatePropagation();ensurePalletField();ensureCementTypeField();
    const raw=String($("cementTonnage")?.value||"").trim(),number=(raw==="-"||raw==="0")?0:Number(raw.replace(",",".")),ton=number===0?null:number,vehicle=parseCount($("cementVehicleCount")?.value),pallet=parseCount($("cementPalletCount")?.value),payload={tarih:$("cementDate")?.value,firma:title($("cementCompany")?.value),cimento_ozelligi:$("cementType")?.value.trim().replace(/\s+/g," ")||null,teslim_yeri:title($("cementDelivery")?.value),arac_sayisi:vehicle,palet_sayisi:pallet,toplam_tonaj:ton,notlar:note($("cementNotes")?.value),tamamlandi:false};
    if(!payload.tarih||!payload.firma||!payload.teslim_yeri||!Number.isFinite(vehicle)||!Number.isFinite(pallet)||(vehicle<=0&&pallet<=0)||(ton!==null&&(!Number.isFinite(ton)||ton<=0))){const s=$("cementStatus");if(s){s.textContent="Şantiye adını ve araç veya palet sayısından en az birini gir. Kullanmadığın sayıya - yazabilirsin.";s.style.color="var(--danger)"}return}
...
(function loadRecordsCementAddon(){if(document.querySelector('script[src^="./records-cement-addon.js"]'))return;const s=document.createElement("script");s.src="./records-cement-addon.js?v=20260925-phone-layout1";s.defer=true;document.head.appendChild(s)})();
(function loadCementPalletExports(){if(document.querySelector('script[src^="./cement-pallet-exports.js"]'))return;const s=document.createElement("script");s.src="./cement-pallet-exports.js?v=20260925-note-case2";s.defer=true;document.head.appendChild(s)})();
