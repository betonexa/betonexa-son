(function(){
  ...
  function render(){const rows=$("cementRows");if(!rows)return;ensurePalletHeader();ensureCementTypeHeader();const items=filtered();rows.innerHTML=items.length?items.map((r,i)=>`<tr><td>${i+1}</td><td>${esc(trDate(r.tarih))}</td><td>${esc(title(r.firma))}</td><td>${esc(r.cimento_ozelligi||"-")}</td><td>Şantiye</td><td>${countText(r.arac_sayisi)}</td><td>${countText(r.palet_sayisi)}</td><td>${r.toplam_tonaj==null?"-":fmt(r.toplam_tonaj)+" ton"}</td><td><div class="cement-row-actions"><button type="button" class="icon-action edit" title="Düzenle" data-cement-edit="${esc(r.id)}">✏️</button><button type="button" class="icon-action delete" title="Sil" data-cement-delete="${esc(r.id)}">🗑️</button></div></td></tr>`).join(""):'<tr><td colspan="9" class="cement-empty">Seçilen dönemde çimento sevkiyatı bulunmuyor.</td></tr>';
