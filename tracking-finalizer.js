...
(function(){
  ...
  function keepFilled(r){clearInterval(fillGuard);fill(r);let rounds=0;fillGuard=setInterval(()=>{rounds++;if(String(editId)!==String(r.id)||rounds>12){clearInterval(fillGuard);fillGuard=null;return;}fill(r);},100);}
  async function openEdit(id){const client=db();if(!client){alert('Veritabanı bağlantısı yüklenemedi.');return;}const {data,error}=await client.from(TABLE).select('*').eq('id',id).single();if(error||!data){alert('Çimento sevkiyatı bulunamadı'+(error?.message?': '+error.message:'.'));return;}if(!showPage()){alert('Çimento düzenleme formu açılamadı.');return;}keepFilled(data);}

  function extractCementId(btn){
    if(!btn)return null;
    if(btn.dataset?.cementEdit)return btn.dataset.cementEdit;
    const row=btn.closest('tr');
    const status=row?.querySelector('.shipment-status-select[data-shipment-type="cement"]');
    if(status?.dataset?.shipmentId)return status.dataset.shipmentId;
    const block=btn.closest('.rc-block');
    const cementBlock=block&&/Çimento/i.test(block.querySelector('h3')?.textContent||'');
    const inCementPage=!!btn.closest('#cementPage');
    if(!cementBlock&&!status&&!inCementPage)return null;
    const code=btn.getAttribute('onclick')||'';
    let m=code.match(/editCementFromCombined\(['"]?([^'")]+)['"]?\)/);
    if(m)return m[1];
    m=code.match(/editCementShipment\(['"]?([^'")]+)['"]?\)/);
    if(m)return m[1];
    m=code.match(/\(['"]([^'"]+)['"]\)/);
    return m?m[1]:null;
  }
  ...
  function isEditButton(btn){
    if(!btn)return false;
    if(btn.matches('.rc-edit,.icon-action.edit,[data-cement-edit]'))return true;
    return /düzenle/i.test(btn.getAttribute('title')||'')||/✏/.test(btn.textContent||'');
  }

  function intercept(event){
    const btn=event.target.closest?.('button,[role="button"]');
    if(!btn||!isEditButton(btn))return;
    if(!btn.closest('#recordsPage')&&!btn.closest('#recordsCombinedView'))return;
    const id=extractCementId(btn);
    if(!id)return;
