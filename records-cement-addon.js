(function(){
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='recordsCementAddonStyles';s.textContent=`
      ...
      .rc-error{padding:12px 14px;margin:0 0 12px;border-radius:12px;background:rgba(204,51,42,.10);color:var(--danger);font-size:13px;font-weight:700}
  ...
  function cementHtml(items){
    ...
    return `<div class="rc-table"><table><thead><tr><th>No</th><th>Tarih</th><th>Firma</th><th>Teslim Yeri</th><th>Araç</th><th>Tonaj</th><th>İşlem</th></tr></thead><tbody>${items.map((r,i)=>`<tr><td>${i+1}</td><td>${esc(trDate(r.tarih))}</td><td>${esc(title(r.firma))}</td><td>${esc(title(r.teslim_yeri||''))}</td><td>${Number(r.arac_sayisi||0)}</td><td>${r.toplam_tonaj==null?'-':fmt(r.toplam_tonaj)+' ton'}</td><td><button type="button" class="rc-edit" onclick="window.editCementFromCombined('${r.id}')">✏️</button></td></tr>`).join('')}</tbody></table></div><div class="rc-total"><span>ÇİMENTO SEVKİYATI: ${items.length}</span><span>TOPLAM ARAÇ: ${vehicles}</span><span>TOPLAM TONAJ: ${fmt(tons)} ton</span>${pending?`<span>TONAJ BEKLEYEN: ${pending}</span>`:''}</div>`;
  }
  ...
  function bind(){
    const range=$('exportRange'),date=$('exportDate'),pdfBtn=$('pdfBtn'),refreshBtn=$('refreshBtn'),loginBtn=$('loginBtn');
    ...
    if(client?.auth?.onAuthStateChange){
      client.auth.onAuthStateChange((event,session)=>{
        if(session&&(event==='SIGNED_IN'||event==='TOKEN_REFRESHED'||event==='INITIAL_SESSION'))setTimeout(refresh,150);
      });
    }

    const app=$('app');
    if(app&&window.MutationObserver){
      new MutationObserver(()=>{if(!app.classList.contains('hidden'))setTimeout(refresh,150)}).observe(app,{attributes:true,attributeFilter:['class']});
    }
  }
  ...
  function init(){styles();bind();setTimeout(refresh,250);setTimeout(refresh,1500)}
