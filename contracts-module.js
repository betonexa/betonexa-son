...
function exportData(){
  ...
  const ce=cementRows().filter(r=>r.style.display!=='none').map((r,i)=>{const tonaj=(r.children[5]?.textContent||'').trim();return{No:i+1,Tarih:(r.children[1]?.textContent||'').trim(),Firma:canonicalLabel(r.children[2]?.textContent||''),'Teslim Yeri':canonicalLabel(r.children[3]?.textContent||''),Araç:(r.children[4]?.textContent||'').trim(),Tonaj:tonaj,Planlanan:tonaj==='-'?null:plannedOf(r,tonaj),Durum:statusOf(r)}});
