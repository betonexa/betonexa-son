...
function unique(rows,i){return [...new Set(rows.map(r=>(r.children[i]?.textContent||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'tr'))}
function fill(id,values,label){const el=$(id);if(!el)return;const cur=el.value;el.innerHTML=`<option value="">Tüm ${label}</option>`+values.map(v=>`<option value="${esc(v)}">${v}</option>`).join('');if([...el.options].some(o=>o.value===cur))el.value=cur}
...
function refreshOptions(){const b=betonBlock();if(!b)return;const rows=[...b.querySelectorAll('.rc-table tbody tr')].filter(r=>r.children.length>=9);fill('shipmentFilterCompany',unique(rows,4),'Firmalar');fill('shipmentFilterSite',unique(rows,5),'Şantiyeler');fill('shipmentFilterPlant',unique(rows,3),'Santraller');fill('shipmentFilterConcrete',unique(rows,6),'Beton Sınıfları')}
...
async function pdf(){const p=exportData();if(!p.rows.length)return alert('Çıktı alınacak filtrelenmiş sevkiyat bulunmuyor.');if(!window.jspdf?.jsPDF)return alert('PDF modülü yüklenemedi.');const{jsPDF}=window.jspdf,doc=new jsPDF({orientation:'landscape',unit:'mm',format:'a4'});doc.setFontSize(14);doc.text('Filtrelenmiş Beton Sevkiyatları',14,14);doc.setFontSize(8);doc.text(filterSummary(),14,20,{maxWidth:265});doc.autoTable({head:[['No','Tarih','Saat','Santral','Firma','Şantiye','Beton','Metraj','Pompa']],body:p.data.map(x=>[x.No,x.Tarih,x.Saat,x.Santral,x.Firma,x['Şantiye'],x.Beton,x.Metraj,x.Pompa]),startY:25,styles:{fontSize:7},headStyles:{fillColor:[111,66,193]}});doc.text(`TOPLAM: ${p.rows.length} sevkiyat · ${trNum(p.total)} m³`,14,(doc.lastAutoTable?.finalY||30)+8);doc.save('Filtrelenmis-Beton-Sevkiyatlari.pdf')}
