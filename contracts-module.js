...
function addStyles(){
  ...
  if(!$('shipmentFilterStyles')){const s=document.createElement('style');s.id='shipmentFilterStyles';s.textContent=`
    ...
    @media(max-width:700px){#shipmentQuickFilters{padding:12px;max-width:100%;overflow:visible;box-sizing:border-box}#shipmentQuickFilters .sqf-grid{grid-template-columns:minmax(0,1fr);width:100%;max-width:100%;min-width:0;box-sizing:border-box}#shipmentQuickFilters .sqf-grid>label{min-width:0!important;max-width:100%!important;width:100%!important;overflow:visible;box-sizing:border-box}#shipmentFilterCompany,#shipmentFilterSite{overflow-y:auto;touch-action:pan-y}#shipmentFilterStart,#shipmentFilterEnd{display:block!important;width:100%!important;max-width:100%!important;min-width:0!important;inline-size:100%!important;max-inline-size:100%!important;min-inline-size:0!important;margin-left:0!important;margin-right:0!important;box-sizing:border-box!important;-webkit-appearance:none!important;appearance:none!important}#shipmentFilterStart::-webkit-date-and-time-value,#shipmentFilterEnd::-webkit-date-and-time-value{width:100%!important;min-width:0!important}#shipmentQuickFilters .sqf-actions .btn{flex:1}#shipmentQuickFilters .sqf-clear{margin-left:0}#shipmentFilterResult{justify-content:flex-start}}
  `;document.head.appendChild(s)}
}
...
function rowIso(row){const t=(row.children[1]?.textContent||'').replace(/\u00a0/g,' ').trim();let m=t.match(/(\d{2})[.\/-](\d{2})[.\/-](\d{4})/);if(m)return m[3]+'-'+m[2]+'-'+m[1];m=t.match(/(\d{4})-(\d{2})-(\d{2})/);if(m)return m[0];const months={ocak:'01',şubat:'02',mart:'03',nisan:'04',mayıs:'05',haziran:'06',temmuz:'07',ağustos:'08',eylül:'09',ekim:'10',kasım:'11',aralık:'12'};m=t.toLocaleLowerCase('tr-TR').match(/(\d{1,2})\s+(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık)\s+(\d{4})/u);return m?m[3]+'-'+months[m[2]]+'-'+String(m[1]).padStart(2,'0'):''}
...
function applyFilters(){
  ...
  const{concrete:cb,cement:ceb}=blocks();let cc=0,m3=0,zc=0,vehicles=0,pallets=0,tons=0;
  ...
  cementRows().forEach(r=>{const ok=type!=='concrete'&&(!company||canonicalKey(r.children[2]?.textContent)===company)&&(!delivery||canonicalKey(r.children[3]?.textContent)===delivery)&&matchesDate(r,start,end);r.style.display=ok?'':'none';if(ok){zc++;vehicles+=parseNumber(r.children[4]?.textContent);pallets+=parseNumber(r.children[5]?.textContent);tons+=parseNumber(r.children[6]?.textContent)}});
  if(cb)cb.style.display=type==='cement'?'none':'';if(ceb)ceb.style.display=type==='concrete'?'none':'';
  const result=$('shipmentFilterResult');if(result){result.style.setProperty('display','none','important');const parts=[];if(type!=='cement')parts.push(`<span class="sqf-badge">🚚 ${cc} beton · ${trNum(m3)} m³</span>`);if(type!=='concrete')parts.push(`<span class="sqf-badge sqf-badge-cement">🏗️ ${zc} çimento · ${vehicles} araç · ${pallets} palet · ${trNum(tons)} ton</span>`);result.innerHTML=parts.join('')}
...
function exportData(){
  const statusOf=row=>row.querySelector('.shipment-status-select')?.value||'planlandi';
  const plannedOf=(row,current)=>{const first=row.querySelector('.shipment-amount-change span')?.textContent||'';return first?parseNumber(first):parseNumber(current)};
  const cr=concreteRows().filter(r=>r.style.display!=='none').map((r,i)=>{const metraj=(r.children[7]?.textContent||'').trim();return{No:i+1,Tarih:(r.children[1]?.textContent||'').trim(),Saat:(r.children[2]?.textContent||'').trim(),Santral:(r.children[3]?.textContent||'').trim(),Firma:canonicalLabel(r.children[4]?.textContent||''),'Şantiye':canonicalLabel(r.children[5]?.textContent||''),Beton:(r.children[6]?.textContent||'').trim(),Metraj:metraj,Planlanan:plannedOf(r,metraj),Durum:statusOf(r),Pompa:(r.children[8]?.textContent||'').trim(),Sorumlu:(r.children[9]?.textContent||'').trim(),Telefon:(r.children[10]?.textContent||'').trim()}});
  const ce=cementRows().filter(r=>r.style.display!=='none').map((r,i)=>{const tonaj=(r.children[6]?.textContent||'').trim();return{No:i+1,Tarih:(r.children[1]?.textContent||'').trim(),Firma:canonicalLabel(r.children[2]?.textContent||''),'Teslim Yeri':canonicalLabel(r.children[3]?.textContent||''),Araç:(r.children[4]?.textContent||'').trim(),Palet:(r.children[5]?.textContent||'').trim(),Tonaj:tonaj,'Çimento Özelliği':(r.children[7]?.textContent||'').trim(),Planlanan:tonaj==='-'?null:plannedOf(r,tonaj),Durum:statusOf(r)}});
  return{concrete:cr,cement:ce,m3:cr.reduce((s,r)=>s+parseNumber(r.Metraj),0),vehicles:ce.reduce((s,r)=>s+parseNumber(r.Araç),0),pallets:ce.reduce((s,r)=>s+parseNumber(r.Palet),0),tons:ce.reduce((s,r)=>s+parseNumber(r.Tonaj),0)}
}
...
function placeFilterExports(){
  ...
  ['pdfBtn','excelBtn','printBtn'].forEach(id=>{const legacy=$(id);if(legacy)legacy.style.setProperty('display','none','important')});const result=$('shipmentFilterResult');if(result)result.style.setProperty('display','none','important');
