(function(){
  ...
  async function pdfReport(cementOnly=false){
    try{
      ...
      if(data.cement.length){if(y>165){doc.addPage();y=18}doc.setFontSize(11);doc.setTextColor(18,25,62);doc.text(text('ÇİMENTO SEVKİYATLARI'),10,y);y+=5;for(const group of cementGroups(data.cement)){if(y>170){doc.addPage();y=18;doc.setFontSize(11);doc.setTextColor(18,25,62);doc.text(text('ÇİMENTO SEVKİYATLARI (DEVAMI)'),10,y);y+=5}const gv=group.items.reduce((s,r)=>s+Number(r.arac_sayisi||0),0),gp=group.items.reduce((s,r)=>s+Number(r.palet_sayisi||0),0),gt=group.items.reduce((s,r)=>s+Number(r.toplam_tonaj||0),0);doc.setFillColor(247,246,252);doc.setDrawColor(211,205,235);doc.roundedRect(10,y,277,7,1,1,'FD');doc.setFontSize(7.5);doc.setTextColor(18,25,62);doc.text(text(group.name),12,y+4.7);doc.setTextColor(74,35,181);doc.text(text(`Firma Toplamı: ${group.items.length} sevkiyat  •  ${gv} araç  •  ${gp} palet  •  ${fmt(gt)} ton`),285,y+4.7,{align:'right'});y+=7;doc.autoTable({startY:y,head:[['No','Tarih','Firma','Çimento özelliği','Teslim Yeri / Şantiye','Araç','Palet','Miktar','Durum'].map(text)],body:group.items.map((r,i)=>[i+1,trDate(r.tarih),label(r.firma),r.cimento_ozelligi||'-',label(r.teslim_yeri||'Şantiye'),showCount(r.arac_sayisi),showCount(r.palet_sayisi),r.toplam_tonaj==null?'Bilgi bekleniyor':`${fmt(r.toplam_tonaj)} ton`,status(r)].map(text)),styles:{font,fontSize:6,cellPadding:1.3},headStyles:{font,fillColor:[74,35,181]},columnStyles:{0:{cellWidth:8,halign:'center'},5:{halign:'center'},6:{halign:'center'},7:{halign:'center'}},margin:{left:10,right:10}});y=(doc.lastAutoTable?.finalY||y+15)+5}const vehicles=data.cement.reduce((s,r)=>s+Number(r.arac_sayisi||0),0),pallets=data.cement.reduce((s,r)=>s+Number(r.palet_sayisi||0),0),tons=data.cement.reduce((s,r)=>s+Number(r.toplam_tonaj||0),0);if(y>190){doc.addPage();y=18}doc.setFontSize(9);doc.setTextColor(74,35,181);doc.text(text(`GENEL TOPLAM ÇİMENTO: ${data.cement.length} sevkiyat   •   ${vehicles} araç   •   ${pallets} palet   •   ${fmt(tons)} ton`),10,y);}
